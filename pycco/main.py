...
#            continue
        ...
        # begin rp
        # Skip over lines that start with either <%--# or #--%>
        # This allows comments in ASPX files -- which must start with //. 
...
#            continue
        ...
        if multistart and multiend \
           and any(line.lstrip().startswith(delim) or line.rstrip().endswith(delim)
            ...
            # begin rp - Treat lines that start with //- as code.
            if line.lstrip().startswith('//-'):            
                process_as_code = True
            else:   
            # end rp - note this addition changes indention of next block.                       
                if has_code:
...
def destination(filepath, preserve_paths=True, outdir=None):
    ...
    if not outdir:
        raise TypeError("Missing the required 'outdir' keyword argument.")
    # rp
    # This chunk removes the exension from the output filename.         
    ...
    #     name = filename
    # rp 
    # I added this line to unconditionally set the output name to the 
    # file name. Pycco appends the .html extension a little later.
    name = filename        
